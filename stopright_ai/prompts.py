...
담당자 기준:
- 배관·서포트·덕트 밟음 계열에서 담당부서 허락·협의·승인은 참고 신호일 뿐 단독 진성 근거가 아니다. 실제 작업상태, 물리적 강제성, 물리적 보강 또는 대체 공법 승인 필요성과 함께 분리한다.
...
- "작업예정" 또는 "사전협의"는 원칙적으로 행정 신호로 분리한다. 다만 작업자가 곧바로 위험구역에 투입될 상태이고, 안전한 발판·지정 안전방법을 사용할 수 없어 대체 공법 승인이나 물리적 보강 없이는 위험 통제가 불가능하다는 직접 근거가 있으면 별도 구분한다.
...
{{
  ...
  "pipe_support_evidence": {{
    ...
    "work_phase_for_pipe_evidence": ["배관·서포트 밟음 이슈의 작업 시점 근거"],
    "actual_or_planned_work": "실제작업중|작업개시직전위험구역진입|작업전현장확인|작업예정문서협의|기간연장요청|불명확",
    "actual_or_planned_work_evidence": ["실제 작업인지, 예정/문서 협의인지 가르는 문구"],
    ...
    "stepping_context_evidence": ["실제 밟음/밟을 수밖에 없음/단순 허가요청을 구분하는 직접 문구"],
    "forbidden_equipment_type": "케미컬배관|정제질소배관|가스배관|Toxic Duct|덕트|서포트|설비프레임|전기설비|일반배관|해당없음|불명확",
    "standard_method_blocked": "예|아니오|불명확",
    "standard_method_blocked_evidence": ["회사 지정 안전도구·작업방법을 현장 조건상 지킬 수 없는 근거"],
    ...
    "permission_required_evidence": ["담당부서 승인 없이는 안전기준·작업방법을 지킬 수 없다는 근거"],
    ...
    "admin_only_evidence": ["작업 전 이메일 협의, 허가 요청, 기간 연장, 일정 조율, 단순 요청완료 등 행정성 근거"],
    "admin_vs_safety_discriminator": "안전통제협의|대체작업방법승인|물리적보강협의|단순행정협의|구분불명확",
    "admin_vs_safety_discriminator_evidence": ["협의가 안전통제인지 행정협의인지 가르는 가장 직접적인 문구"],
    "image_pipe_findings": ["이미지에서 직접 보이는 발판 부재, 배관/서포트 밟음, 보강판, 비계, 공간 협소, 작업자 위치. 확인 불가하면 빈 배열"],
    "pipe_true_discriminator": ["해당하는 진성 구분자만 선택: 작업중실제밟음, 밟을수밖에없음, 안전발판부족, 금지설비밟음, 대체공법승인_물리강제성동반, 물리적보강필요, 비계보강판설치, 작업방법변경"],
    "pipe_false_guardrail": ["해당하는 배관계열 가성 신호만 선택: 작업예정, 사전협의, 기간연장, 단순허가요청, DRI작성, 이메일공유, 파손유의, 단순이메일협의, 단순일정조율, 실제밟음불명확, 작업자노출불명확, 물리적보강없음, 태그단독_DS조치건, 태그단독_협력회사자체개선완료, 태그단독_요청완료, 태그단독_조치완료"],
    "decisive_pipe_support_evidence": ["pipe/support 판정을 가르는 직접 문구. 없으면 빈 배열"]
  }},
