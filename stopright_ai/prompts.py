...
- 접액·누출 계열은 물질명보다 작업중지 당시 실제 액체·누수·응축수·냄새·가스·접액 우려가 있었고 작업자가 안전성을 확신할 수 없어 멈췄는지가 핵심 구분인자다.
...
- 작업중지권 등록 건에서는 사후에 DIW·일반 물·응축수로 확인되었는지보다, 작업중지 당시 작업자가 안전성을 확신할 수 없어 운영부서·담당부서 확인 후 재개했는지를 우선 구분한다.
- 위험 표현이 있더라도 예비품·공구·릴선·대차 교체, 재체결, 청소, 위치 이동처럼 운영부서 승인이나 전문조치 없이 현장에서 짧은 시간 내 단일 조치로 끝나는지 구분한다.
...
{{
  ...
  "simple_correction_evidence": ["단순 청소, 배수, 정리정돈, 위치변경, 교육, 예비품·공구·릴선·대차 교체, 재체결 등 근거"],
  "quick_fix_without_external_control": "예|아니오|불명확",
  "quick_fix_evidence": ["운영부서 승인·전문조치 없이 1~2시간 내 현장 단일조치로 해결 가능한지 근거"],
  ...
  "false_positive_guardrails": ["해당하는 가성 신호만 선택: 작업전발견, 작업예정, 사전협의, 허가요청, 기간연장, 실제물질없음, 센서테스트, SW경고, 계획정비알람, 단순교체교육, 행정절차"],
  ...
  "leak_contact_evidence": {{
    ...
    "material_or_contact_status_at_stop": "실제액체발견|누수발견|응축수발견|접액우려|냄새가스발견|약품흔적발견|센서경고만|SW경고만|계획정비알람만|해당없음|불명확",
    "material_or_contact_evidence": ["작업중지 당시 실제 물질 또는 접액 우려 근거"],
    ...
    "stop_reason_uncertainty": "작업자가 당시 안전성을 확신할 수 없어 멈춘 정황 있음|없음|불명확",
    "leak_false_guardrail": ["해당하는 접액계열 가성 신호만 선택: 실제물질없음, 센서테스트, SW경고, 계획정비알람"]
  }},
