...
def build_report(input_path: Path, base_metrics: dict, summary: pd.DataFrame) -> str:
    lines = [
        ...
        "## Gate Profiles",
        "",
        "- `strict`: flips only when the row is admin/prework/weak-force/no-countermeasure with no positive physical signal.",
        "- `balanced`: flips pipe/support rows with no positive physical signal and multiple negative/admin signals.",
        "- `approval_only`: targets approval/admin-only cases without active phase, force, or countermeasure support.",
        "",
        "## Metrics",
        "",
    ]
